...
create table if not exists public.circuit_component_asset_images (
  ...
  storage_path text,
  external_url text,
  mime_type text check (mime_type is null or mime_type in ('image/png', 'image/jpeg')),
  width_px integer check (width_px is null or width_px > 0),
  height_px integer check (height_px is null or height_px > 0),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint circuit_component_asset_images_source_check check (
...
create table if not exists public.circuit_component_pins (
  ...
  signal_type text not null check (
    signal_type in ('analog', 'component', 'digital', 'ground', 'gpio', 'i2c', 'power', 'pwm', 'spi', 'uart')
  ),
...
create index if not exists circuit_component_assets_category_idx
  on public.circuit_component_assets(category);

create index if not exists circuit_component_pins_component_order_idx
  on public.circuit_component_pins(component_id, sort_order);
...
create policy "Public read ready circuit assets"
on public.circuit_component_assets
for select
to anon, authenticated
using (status = 'ready');

drop policy if exists "Public read images for ready circuit assets" on public.circuit_component_asset_images;
create policy "Public read images for ready circuit assets"
on public.circuit_component_asset_images
for select
to anon, authenticated
using (
  exists (
...
create policy "Public read pins for ready circuit assets"
on public.circuit_component_pins
for select
to anon, authenticated
using (
  exists (
...
create policy "Public read circuit asset files"
on storage.objects
for select
to anon, authenticated
using (bucket_id = 'circuit-assets');
